...
void ULevelManager::HandleLevelStreamming()
{
	...
	TArray <FVector> CurrentPlayerLocations = GetPlayersPawnsLocations();
	if (CurrentPlayerLocations.Num() <= 0)
	{
		return;
	}	

	TArray<int32> ReservedVisibleLevels = GetLevelPlacementDataForVisibleCells();//Forced visib by external configuration
	
	TArray<int32> AllVisibleLevels= TArray<int32>();
	if (ActorsCanShowLevels)
	{		
		TArray <FVector> CurrentPawnsLocations = GetNonPlayersPawnsLocations();
...
TArray <FVector> ULevelManager::GetPlayersPawnsLocations() const
{
...
TArray <FVector> ULevelManager::GetNonPlayersPawnsLocations() const
{
	if (!ActorsCanShowLevels)
	{
		return TArray <FVector>();
...
TArray<FIntPoint> ULevelManager::GetLevelCellsIDs(int LevelPlacementDataIn) const
{
	...
	if (!LevelPlacementData.IsValidIndex(LevelPlacementDataIn))
	{
		return Cells;
	}		
...
{	
	...
	int32 CorridorsAmount = CircularCorridorsAmountPercent * GeneratedRoomsAmount;
	int32 RandomRoll = 0;
...
void ULevelManager::ChangeLevelVisibibility(int LevelPlacementDataIndex, bool IsVisible)
{
	...
	{		
		...
		else
		{
			...
			if (IsGeneratingLevelsPool)
			{
				UpdateLevelTrack(LevelPlacementDataIndex, nullptr, ELevelGen_LevelState::Unloaded, false); //Use a null pointer for the level here. This level has no valid level now since is now in the pool
			}
			else
			{
...
void ULevelManager::OnCorridorsLayoutDataGenerated()
{
	...
	CellsData.Append(WallsLayoutData.CellsData); //Append walls cells
...
bool ULevelManager::PropagateBiomesFromAdyacentTransitionCells(TArray <FLevel_TransitionCellData>& TransitionDataOut) const
{
	...
	return PropagatedQuad; //Return wheter it could update at least one quad or not
}
...
void ULevelManager::GenerateInitialSpawnActorsTrack(int32 SpawnActorDataIndex)
{
	...
	for (int i = 0; i < SpawnsAmount; i++)
	{
		...
		{		
			...
			{
				//Randomize a cell from the cell data array  @TODO: pick only non-blocking cells, to ensure better spawn.
...
void ULevelManager::GenerateTrackForSingleSpawnActor(int32 SpawnActorDataIndex, FIntPoint CellIDIn, const FLevel_ActorSpawnData& SpawnDataIn)
{
	...
	TrackLocal.Anchor.X = UKismetMathLibrary::RandomFloatInRange(SpawnDataIn.LocationAnchor_Min.X, SpawnDataIn.LocationAnchor_Max.X); //Use same anchors but now for cell //TODO: could have different anchors for cell, but is not really important.
...
FTransform ULevelManager::GetSpawnActorTransform(const FLevel_SingleActorSpawnTrack& TrackIn, bool& SuccesOut) const
{
	...
	GetOwner()->GetWorld()->LineTraceSingleByChannel(TraceHitResults, traceStart_, traceEnd_, ECollisionChannel::ECC_GameTraceChannel1);//@TODO: This will not work if u are not using landscapes, trace channel should be a parameter.
