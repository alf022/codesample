...
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FNoParamsDelegateLevelManager);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FOnLevelVisibilityChanged, FLevel_ClientData, LevelClientData, bool, IsVisible);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnLevelGenerationStateMessage, const FString&, Message);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnTransitionCellsDataGenerated, const FBiomeManagerInitializationData&, TransitionInitDataOut);
...
public:
	...
	FNoParamsDelegateLevelManager OnAllLevelsLoaded;
	
	/** Called when the level is fully generated and ready to be used.*/
	UPROPERTY(BlueprintAssignable, Category = "Level Generation")
	FNoParamsDelegateLevelManager OnLevelGenerationCompleted;

	/** Called when a level changes visiblity.*/
	UPROPERTY(BlueprintAssignable, Category = "Level Generation")
	FOnLevelVisibilityChanged OnLevelVisibilityChanged;

	/** Called when there are at least one new level that is changed visible.*/
	UPROPERTY(BlueprintAssignable, Category = "Level Generation")
	FNoParamsDelegateLevelManager OnNewLevelShown;

	/** Called when there is a new state message.*/
	UPROPERTY(BlueprintAssignable, Category = "Level Generation")
	FOnLevelGenerationStateMessage OnLevelGenerationStateMessage;

	/** Called when the transition cells data is generated.*/
	UPROPERTY(BlueprintAssignable, Category = "Level Generation")
	FOnTransitionCellsDataGenerated OnTransitionCellsDataGenerated;
...
private:
	...
	*/
	...
	TArray <FVector> GetPlayersLocations() const;
	
	TArray <FVector> GetNonPlayerPawnsLocations() const;
	...
	/** Processing of data generate by the thread.*/
	...
	/** Finish stream load levels that are needed.*/
	...
	*/
	...
	/** Shows or hides the tracked spawned actor. If needed, it also spawns it for first time.*/
