...
{
	FIntPoint StartCell = GetStartCellID();
	FVector StartLocation = ULevelGeneratorFunctionLibrary::GetCellWorldPosition(StartCell, CellSize, InitialLocation);
...
void ULevelManager::HandleLevelStreamming()
{
	...
	TArray <FVector> CurrentPlayerLocatios = TArray <FVector>();
	for (int i = 0; i < PlayerStates.Num(); i++)
	{
		if (PlayerStates[i]->GetPawn())
		{
			CurrentPlayerLocatios.Add(PlayerStates[i]->GetPawn()->GetActorLocation());
		}
	}

	if (CurrentPlayerLocatios.Num() <= 0)
	{
		return;
	}	
	...
	TArray<int32> AllVisibleLevelsPawns = TArray<int32>();
	if (ActorsCanShowLevels)
	{
		TArray <AActor*> LocalActors = TArray <AActor*>();
		UGameplayStatics::GetAllActorsOfClass(this, StreamActorClass, LocalActors);//@TODO too expensive. It is ok because the tick time is slow, but might need to change this in the future
		TArray <FVector> CurrentPawnsLocatios = TArray <FVector>();

		for (int i = 0; i < LocalActors.Num(); i++)
		{
			CurrentPawnsLocatios.Add(LocalActors[i]->GetActorLocation());
		}

		AllVisibleLevelsPawns = GetLevelPlacementDataBetweenPositions(CurrentPawnsLocatios, 0, CellSize, ReservedVisibleLevels);//Calculate for any pawn with minimal distance. This is for any walking pawn.
	}
	
	ProcessAllCellVisibilityStatus();		
	TArray<int32> MustLoadLevels = GetLevelPlacementDataBetweenPositions(CurrentPlayerLocatios, 0, LoadStreamLevelDistance, TArray<int32>());// AllVisibleLevelsPawns);//Calculate for players with a distance that hides the level borders.
	TArray<int32> CanRemainLoadLevels = GetLevelPlacementDataBetweenPositions(CurrentPlayerLocatios, LoadStreamLevelDistance, LoadStreamLevelDistance + UnloadStreamLevelTolerance, TArray<int32>());
...
FIntPoint ULevelManager::GetClosesCellIDToAnchorPoint(const FLayoutData& LayoutDataIn, FVector2D AnchorIn) const
{
	...
	//Need to get size and location from cell data, since the layout might not be "tight" to the cells
...
FLevel_RoomDistance ULevelManager::GetMinimunRoomDistancePaths(const TArray<int32>& ThreeRoomsIn, const TArray<FLevel_RoomDistance>& RoomsDistanceIn, int32& RoomDistanceIndexOut) const
{
	...
	{
		//One room is in the three and the other is not?
...
FVector ULevelManager::GetPlacedLevelPosition(int LevelPlacementDataIndex) const
{
	...
	CurrentLevelLocation.X = LevelPlacementData[LevelPlacementDataIndex].GridPosition.Y * CellSize + InitialLocation.X;
	...
	{
	case 1: CurrentLevelLocation.X = CurrentLevelLocation.X + CurrenLevelSize * CellSize ;
		break;
	case 2:
		...
		break;
	case 3: CurrentLevelLocation.Y = CurrentLevelLocation.Y + CurrenLevelSize * CellSize ;
		break;
	default:
		break;
	}
...
int32 ULevelManager::GetLevelsPoolNumber()
{
	...
	UE_LOG(LevelGeneratorLog, Log, TEXT("ULevelManager::GetLevelsPoolNumber - Levels Pool Size: %i."), LevelsPooled);
	return LevelsPooled;
}
...
int32 ULevelManager::GetLevelDataPoolMinimunLevels(int32 LevelDataIndexIn) const
{
	...
	for (int32 i = 0; i < LevelPlacementData.Num(); i++)
	{
		if (LevelPlacementData[i].LevelDataIndex == LevelDataIndexIn)
		{
			...
			{
				MinLevelsCounter = LevelsInRadius; // this is now the min levels together
			}
		}
	}
...
void ULevelManager::CreateLevelInstance(int LevelPlacementDataIndex, bool ForceAsyncOnLoad)
{
	...
	else
	{
		LevelDynamic = ULevelStreamingDynamic::LoadLevelInstance(this, LevelsData[LevelPlacementData[LevelPlacementDataIndex].LevelDataIndex].LevelName,
			GetPlacedLevelPosition(LevelPlacementDataIndex), GetPlacedLevelRotation(LevelPlacementDataIndex), Succes);

		UE_LOG(LevelGeneratorLog, Log, TEXT("ULevelManager::CreateLevelInstance - Level instance created: %s, for map name: %s."),
...
FLevel_ClientData ULevelManager::GetLevelClientData(int LevelPlacementDataIndexIn) const
{
	...
	for (int i = 0; i < TransitionCellsData.Num(); i++)
	{
		if (TransitionCellsData[i].CellData.LevelPlacementDataIndex == LevelPlacementDataIndexIn)
		{
			...
			break; //Cuz transition levels are only 1 cell, i can do this
		}
	}
		
	if (!Data.IsTransition) //if not transition, biome up is the level biome
	{
		for (const FCellData& cell : CellsData)
		{
			if (cell.LevelPlacementDataIndex == LevelPlacementDataIndexIn)
			{
				Data.Biome_Up = cell.Cell_Biome;
				break; //All the level have the same biome.
			}
		}	
	}
...
{
	if (IsGeneratingLevelsPool) return;
...
{	
	...
	//If hits nothing then can hide cell
	return (ActorsHit.Num() <= 0);
}
...
void ULevelManager::ChangeLevelVisibibility(int LevelPlacementDataIndex, bool IsVisible)
{
	...
	{		
		...
		else //if(CanHideLevel(LevelPlacementDataIndex))
		{
...
void ULevelManager::ClearLevel()
{
	...
	ClearLevelsPool(); //@TODO I could not destroy pools once generated or do it in a smart way that avoids creating so many instances again, etc, But since i dont call this function repetedly, is not big deal for now.
...
void ULevelManager::HandleRoomLayoutThreads()
{
	...
	if (AreAllRoomLayoutThreadsCompleted())
	{
		...
		ULevelGeneratorFunctionLibrary::RemoveRepeatedCellsFromLayouts(LayoutData, TArray<FLayoutData>()); //because this are the rooms, the celldata array is empty.
		...
		GeneratedRoomsAmount = LayoutData.Num(); 
...
TArray<FGameplayTag> ULevelManager::GenerateBiomesForAreas(int32 AreasAmountIn)
{
	...
	{
		if (UnusedBiomes.Num() <= 0)
		{
			UnusedBiomes = PossibleBiomes;
		}
...
void ULevelManager::StartCorridorsLayoutDataGeneration()
{
	...
	//This is the amount of corridors per thread. The Idea is to not mass too many threads. The threads handle a few corridors
...
void ULevelManager::OnCorridorsLayoutDataGenerated()
{
	...
	CellsData.Append(WallsLayoutData.CellsData);

	TArray<FCellData> MutatedCells = MutateTransitionCells(CellsData);
	UpdateMutatedCellsInLayouts(MutatedCells, LayoutData);
	UpdateMutatedCellsInLayouts(MutatedCells, CorridorsLayoutData);
	LayoutData.Append(CorridorsLayoutData);

	TArray <FLayoutData> WallLayout = TArray <FLayoutData>(); 
...
void ULevelManager::UpdateBiomesForWallCellsData(TArray<FCellData>& CellsDataOut)
{
	...
	while (HasAnyCellWithNoBiome(CellsDataOut) && whilecounter < 200) 
	{
...
void ULevelManager::FinishStreamLoadStartLevels()
{
	...
	InitData.LoadedLevelsClientData = GetLoadedLevelsClientData(); //Get loaded data AFTER transition cells data update
			...
			if (!IsBiomeValid(GetQuadBiomeInDirection(Direction, TransData)))
			{
				...
				case ELevelGen_QuadPropagationMode::Random:
				{
					if (ValidBiomeCount == 2)
					{
						if (QuadAdyacentTags[0] != QuadAdyacentTags[1])
						{
							SetQuadBiomeInDirection(Direction, GetRandomValidBiomeInArray(QuadAdyacentTags), TransData);
							PropagatedQuad = true;
							break;
						}
					}
...
FGameplayTag ULevelManager::GetAdyacentCellBiomeInDirection(ELevelGen_CorridorDirection DirectionIn, const FCellData& CellIn) const
{
	...
	FGameplayTag AdyBiome = FGameplayTag();
	if (CellsData.IsValidIndex(CellIndex))
	{	
...
FGameplayTag ULevelManager::GetAdyacentBiomeOfQuadInDirection_InAdyacentCell(ELevelGen_CorridorDirection DirectionIn, const FLevel_TransitionCellData& CellTransDataIn, const TArray<FLevel_TransitionCellData>& TransitionDataIn) const
{
	...
	switch (DirectionIn) //opposite direction for cell check
	{
	case ELevelGen_CorridorDirection::Up:
...
{
	if (!ActorsSpawnData.IsValidIndex(SpawnActorDataIndex))
	{
		return;
	}	
	...
	for (int i = 0; i < SpawnsAmount; i++)
	{
		...
		{		
			...
			case ELevelGen_ActorSpawnType::Anywhere:
			{
				...
				if (!CellsData.IsValidIndex(RandomCell))
				{
					return;
				}
			...
			{
				int32 StartRoomLocal = GetClosestLayoutIndexToAnchor(LayoutData, ELevelGen_LevelType::Normal, StartRoomAnchor);
				for (int j = 0; j < LayoutData.Num(); j++)
				{
					if (j != StartRoomLocal)
					{
...
void ULevelManager::GenerateTrackForSingleSpawnActor(int32 SpawnActorDataIndex, FIntPoint CellIDIn, const FLevel_ActorSpawnData& SpawnDataIn)
{
	...
	TrackLocal.Anchor.X = UKismetMathLibrary::RandomFloatInRange(SpawnDataIn.LocationAnchor_Min.X, SpawnDataIn.LocationAnchor_Max.X); //Use same anchors but now for cell. Should have different anchors for cell?
